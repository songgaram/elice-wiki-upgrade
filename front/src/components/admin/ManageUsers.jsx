...
const ManageUsers = () => {
  ...
  const checkAll = (e) => {
    if (e.target.checked) {
      const idList = data.users.map((datum) => datum.__id);
  ...
  const controller = async (e) => {
    ...
    if (e.target.name === "deleteUser") {
      ...
      const { data } = await Api.put(`users/${checkedIdString}`, { admin: 0 });
            ...
            data.users.map((datum, index) => {
              return (
                <Tr key={`users/${index}`} color={checkedList.includes(datum.__id) ? "lightgray" : "white"}>
                  <Td>
                    <input type="checkbox" value={datum.__id} onChange={checkHandler} checked={checkedList.includes(datum.__id) ? true : false} />
                  </Td>
                  <Td>{datum.__id}</Td>
                  <Td>{datum.name}</Td>
                  <Td>{datum.email}</Td>
